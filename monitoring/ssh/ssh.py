...
def is_ip_banned(ip):
    ...
    try:
        ...
        send_email(f"Error checking if IP ||{ip}|| is banned: {e}")
...
def ban_ip(ip):
    ...
    try:
        ...
        send_email(f"Error banning IP {ip}: {e}")

        return
    if ssh_notifications:
        send_to_discord(f"IP ||{ip}|| banned for {ssh_ban_duration} minutes due to SSH brute force attempts.")
        send_email(f"IP ||{ip}|| banned for {ssh_ban_duration} minutes due to SSH brute force attempts.")
        ...
        for line in lines:
            if f"DROP" in line and ip in line and "ssh_attempts" in line:
                ...
                if ssh_notifications:
                    ...
                    send_email(f"IP ||{ip}|| has been unbanned after {ssh_ban_duration} minutes.")
                return
        
        send_to_discord(f"Error: IP ||{ip}|| was not found in the current iptables rules for unbanning.")
        send_email(f"Error: IP ||{ip}|| was not found in the current iptables rules for unbanning.")
    except subprocess.CalledProcessError as e:
        send_to_discord(f"Error unbanning IP {ip}: {e}")
        send_email(f"Error unbanning IP {ip}: {e}")
